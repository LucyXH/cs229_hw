...
Transition = np.zeros((NUM_STATES, NUM_STATES, 2)) + 1. / NUM_STATES
# observed params
R_observe = np.zeros(NUM_STATES)
Trans_observe = np.zeros((NUM_STATES, NUM_STATES, 2))
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # model.
    ###### BEGIN YOUR CODE ######
    V_0 = np.dot(Transition[state, :, 0], Value)
    V_1 = np.dot(Transition[state, :, 1], Value)
    if V_0 > V_1:
        action = 0
    else:
        action = 1
    ...
    ###### BEGIN YOUR CODE ######
    Trans_observe[state, new_state, action] += 1
    R_observe[new_state] += R
    ...
    if new_state == NUM_STATES - 1:
        ...
        for s in range(NUM_STATES):
            for a in range(2):
                total = Trans_observe[s, :, a].sum()
                if total != 0:
                    Transition[s, :, a] = 1. * Trans_observe[s, :, a] / total

            total = Trans_observe[:, s, :].sum()
            if total != 0:
                Reward[s] = 1. * R_observe[s] / total
        ...
        while max_change > TOLERANCE:
            # synchronize value iteration
            iteration += 1
            V_0 = np.dot(Transition[:, :, 0], Value)
            V_1 = np.dot(Transition[:, :, 1], Value)
            new_Value = Reward + GAMMA * np.vstack((V_0, V_1)).max(axis=0)
            max_change = np.fabs((new_Value - Value)).max()
            Value = new_Value
        if iteration == 1:
